    ...
    <>
      ...
      {movieCast.slice(0, 8).map((castMember) => (
        <a
          ...
          title="Portami alla pagina dell'attore"
          href={`https://www.themoviedb.org/person/${castMember.id}`}
          key={castMember.cast_id}
        >
          <CastCard person={castMember} type="cast" />
        </a>
      ))}
      <Card id="MoreCastCard">
        <a
          target="_blank"
          rel="noopener noreferrer"
          title="Portami alla pagina con tutto il cast"
          href={`https://www.themoviedb.org/movie/${movieId}/cast`}
        >
          <Card.Body>View more...</Card.Body>
        </a>
      </Card>
    </>
  );
    ...
    <>
      {fetchedDataComplete ? (
        movieDataTMDB.length !== 0 && movieCast.length !== 0 ? (
            ...
            <Row id="RowMovieData">
              <Col lg={4}>
                <Row id="ImagePoster" className="sticky">
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    title="Portami alla pagina del film"
                    href={`https://www.themoviedb.org/movie/${movieDataTMDB.id}?language=it-IT`}
                  >
                    <Image
                      src={
                        "https://image.tmdb.org/t/p/w300" +
                        movieDataTMDB.poster_path
                      }
                    ></Image>
                  </a>
                </Row>
                <div className="stickyCast">
                  <h2 className="TitoloCentrato ">Cast:</h2>
                  <Row id="MovieCastSticky">
                    <CastList
              ...
              <Col lg={8} id="MovieInfo" style={{ margin: "auto" }}>
                <div className="sticky" id="MovieInfoSticky">
                  <Row>
                    <div id="MovieTitle">{movieDataTMDB.title}</div>
                  </Row>

                  <Row>
                    <div id="MovieRandomInfo">
                      <Image src={"/assets/tomato.svg"}></Image>
                      {movieDataTMDB.vote_average.toString().length < 3
                        ? movieDataTMDB.vote_average + ".0"
                ...
                {/* Movie Description*/}
                <div id="MovieDescription">{movieDataTMDB.overview}</div>
                {/* Table Data */}
                <h2
                  style={{
                    margin: "auto",
          ...
          <>
            <h2
              style={{ color: "red", textAlign: "center", marginTop: "40px" }}
            >
            ...
            <div>
              <Row>
                <Col lg={7} id="MovieInfo" style={{ margin: "auto" }}>
                  <div className="sticky" id="MovieInfoSticky">
                    <Row>
                      <div id="MovieTitle">{movieDataMongo.titolo}</div>
                    </Row>
                  </div>
